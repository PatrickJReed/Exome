...
my $sample1_1 	= "2011-929_110823_SN484_0112_AC029KACXX_2_1_sequence.txt.gz";
my $sample1_2 	= "2011-929_110823_SN484_0112_AC029KACXX_2_2_sequence.txt.gz";
my $sample2_1	= "2011-929_120228_SN484_0156_AD0MDRACXX_1_1_sequence.txt.gz";
my $sample2_2	= "2011-929_120228_SN484_0156_AD0MDRACXX_1_2_sequence.txt.gz";
my $sample3_1	= "2011-929_120228_SN484_0156_AD0MDRACXX_2_1_sequence.txt.gz";
my $sample3_2	= "2011-929_120228_SN484_0156_AD0MDRACXX_2_2_sequence.txt.gz";
...
#`$seqprep -6 -f $dir$sample3_1 -r $dir$sample3_2 -1 $ID.3_1.txt.gz -2 $ID.3_2.txt.gz -s $ID.3.txt.gz`;
...
#		`$bwa aln -t 8 -f $ID.3_1.sai $ref_fa $ID.3_1.txt.gz`;
#		`$bwa aln -t 8 -f $ID.3_2.sai $ref_fa $ID.3_2.txt.gz`;
...
#		 `$bwa sampe -n 10 -a 1000 $ref_fa $ID.3_1.sai $ID.3_2.sai $ID.3_1.txt.gz $ID.3_2.txt.gz | 
#		 $samtools view -F 4 -bS - > $ID.3.PE.bam`; 
...
#		 `$bwa aln -t 8 $ref_fa $ID.3.txt.gz | $bwa samse -n 10 $ref_fa - $ID.3.txt.gz | 
#		 $samtools view -F 4 -bS - > $ID.3.SE.bam`;
		...
		`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $picard/SortSam.jar I=$ID.3.PE.bam O=$ID.3.PE.srt.bam SO=coordinate`;										
		...
		`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $picard/SortSam.jar I=$ID.3.SE.bam O=$ID.3.SE.srt.bam SO=coordinate`;			
			
		
			
# Merge clipped and merged alignments
	
	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $picard/MergeSamFiles.jar I=$ID.1.PE.srt.bam I=$ID.2.PE.srt.bam I=$ID.3.PE.srt.bam I=$ID.1.SE.srt.bam I=$ID.2.SE.srt.bam I=$ID.3.SE.srt.bam O=$ID.srt.bam SO=coordinate AS=true USE_THREADING=true`;
