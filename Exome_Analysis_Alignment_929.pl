...
my $sampleQC 	= "2011-929_110624_SN673_0077_AB079HABXX_1_sequence.txt.gz";
...
#prep PE reads, merge and clip
	...
	#Merge PE Reads, remove duplicates
		...
		`java -Xmx8g -jar $picard/MergeSamFiles.jar I=$ID.1.clip.srt.bam I=$ID.2.clip.srt.bam I=$ID.3.clip.srt.bam O=$ID.clip.srt.bam SO=coordinate AS=true USE_THREADING=true`;
	...
	# Merge SE reads, remove duplicates	
		...
		`java -Xmx8g -jar $picard/MergeSamFiles.jar I=$ID.1.mrg.srt.bam I=$ID.2.mrg.srt.bam I=$ID.3.mrg.srt.bam I=$ID.QC.srt.bam O=$ID.mrg.srt.bam SO=coordinate AS=true USE_THREADING=true`;
