...
#	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T HaplotypeCaller -L chr11 -log $ID.hap_chr11_log --dbsnp $dbsnp_vcf -I $K87_450 -I $K87_452 -I $K87_925 -I $K87_926 -I $K87_927 -I $K87_929 -I $K87_930 -o $ID.chr11.haplotypes.vcf -maxAltAlleles 8`;
	...
	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T VariantRecalibrator -log $ID.vr.INDEL_log -resource:hapmap,known=false,training=true,truth=true,prior=15.0 $hapmap -resource:OMNI,known=false,training=true,truth=false,prior=12.0 $OMNI -resource:dbsnp,known=true,training=false,truth=false,prior=6.0 $dbsnp_vcf -input $ID.haplotypes.vcf -mode INDEL -mG 4 --percentBadVariants 0.05 -recalFile $ID.INDEL.recal -tranchesFile $ID.INDEL.tranches -an QD -an MQRankSum -an ReadPosRankSum -an FS -an MQ`;
	
	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T ApplyRecalibration -log $ID.vr.INDEL_log2 --input $ID.haplotypes.vcf --mode INDEL -recalFile $ID.INDEL.recal -tranchesFile $ID.INDEL.tranches -o $ID.recal_Hap.INDEL.vcf`;
