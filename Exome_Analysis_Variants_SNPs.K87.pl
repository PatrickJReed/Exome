...
#	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T HaplotypeCaller -L chr11 -log $ID.hap_chr11_log --dbsnp $dbsnp_vcf -I $K87_450 -I $K87_452 -I $K87_925 -I $K87_926 -I $K87_927 -I $K87_929 -I $K87_930 -o $ID.chr11.haplotypes.vcf -maxAltAlleles 8`;

#		`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T VariantRecalibrator -log $ID.vr.SNP_log -resource:hapmap,known=false,training=true,truth=true,prior=15.0 $hapmap -resource:OMNI,known=false,training=true,truth=false,prior=12.0 $OMNI -resource:dbsnp,known=true,training=false,truth=false,prior=6.0 $dbsnp_vcf --input $ID.haplotypes.vcf --mode SNP -recalFile $ID.SNP.recal -tranchesFile $ID.SNP.tranches -an QD -an MQRankSum -an ReadPosRankSum -an FS -an MQ`;
	
#	`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T ApplyRecalibration -log $ID.vr.SNP_log2 --input $ID.haplotypes.vcf --mode SNP -recalFile $ID.SNP.recal -tranchesFile $ID.SNP.tranches -o $ID.recal_Hap.SNP.vcf`;


`java -Xmx8g -Djava.io.tmpdir=./tmp -jar $gatk -R $ref_fa -T VariantAnnotator -I $K87_450 -I $K87_452 -I $K87_925 -I $K87_926 -I $K87_927 -I $K87_929 -I $K87_930 -o $ID.recal_Hap.SNP.depth.vcf -A DepthPerAlleleBySample -V $ID.recal_Hap.SNP.vcf --dbsnp $dbsnp_vcf`;